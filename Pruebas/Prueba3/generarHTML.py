...
conn = sqlite3.connect('E:\Adrián\Documentos\Argentina programa 4.0\PaginaGalpon\Prueba3\ejemplo.db')
...
def generar_subcategorias(id_padre):
    ...
    for subcategoria in subcategorias:
        html += '<li>'
        print(html)
        # Si la subcategoría tiene subcategorías hijas, agregar la clase "dropdown-submenu"
        if generar_subcategorias(subcategoria[0]):
            html += "<a class='nav-link dropdown-toggle' href='{}'>{}</a>".format(
...
def generar_subcategorias(id_padre):
    ...
    for subcategoria in subcategorias:
        ...
        # Si la subcategoría tiene subcategorías hijas, agregar la clase "dropdown-submenu"
        if generar_subcategorias(subcategoria[0]):
            html += "<a class='dropdown-item dropdown-toggle' href='{}'>{}</a>".format(
                subcategoria[2], subcategoria[1]
            )
            html += generar_subcategorias(subcategoria[0])
        # Si la subcategoría no tiene subcategorías hijas, agregar solo un enlace
